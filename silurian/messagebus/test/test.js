...
var caseBatchProducer = () => {
  let counter = 0
  var producer = new messagebus.BatchProducer('UID_1', (data, callback) => {
    counter++
    if(counter===1) {
...
var caseBatchDefault = () => {
  var producer = new messagebus.BatchProducer('UID_1', (data, callback) => callback(1))
  producer.push({ a: 0 })
  .then(() => producer.push({ a: 1 }))
  .then(() => producer.push({ a: 2 }))
  .then(() => producer.push({ a: 3 }))
  ...
  let counter = 0
  var consumer = new messagebus.BatchConsumer((data, done, uid) => {
    assert(uid==='_uid3_')
    counter++
    if(counter>1) {
      checkError('invalid invocation')
...
var caseBatchConsumerDataloss = () => {
  let counter = 0
  var buffer = []
  var uid2 = 'UID_4'   

  var consumer = new messagebus.BatchConsumer((data, done) => {
    buffer.push(data) 
    assert(!done)
    counter++
    if(counter>8) {
...
var caseBrokerToManySubscriptions = () => {
  var broker = new messagebus.Broker({ maxPendingSubscriptions: 4 })
...
var caseBrokerBasics = () => {
  var broker = new messagebus.Broker()

  let counter = 0

  var uidA = broker.subscribe((err, data, uid) => {
  ...
  var uidB = broker.subscribe((err, data, uid) => {
  ...
  var uidC = broker.subscribe((err, data, uid) => {
